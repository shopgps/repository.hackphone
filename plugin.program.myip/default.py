...
def Network():
    ...
    for ip, region, country, isp in match:
        if inc <2:
            addDir('[B][COLOR gold]Địa chỉ IP bên ngoài: [/COLOR][/B]' + ip,'','',icon,'',fanart)
            addDir('[B][COLOR gold]IP có trụ sở tại: [/COLOR][/B]' + country,'','',icon,'',fanart)
            addDir('[B][COLOR gold]Nhà cung cấp dịch vụ:[/COLOR][/B] ' + isp,'','',icon,'',fanart)
            inc = inc+1
    addDir('[B][COLOR gold]Địa chỉ IP nội bộ:[/COLOR][/B] ' + exip,'','',icon,'',fanart)
    addDir('[B][COLOR gold]Tên của mạng là:[/COLOR][/B] ' + fname,'','',icon,'',fanart)
    addDir('[B][COLOR gold]Network Gateway IP Address is:[/COLOR][/B] ' + gateway,'','',icon,'',fanart)
    addDir('[B][COLOR gold]Network DNS 1 Address is:[/COLOR][/B] ' + dns1,'','',icon,'',fanart)
    addDir('[B][COLOR gold]Network DNS 2 Address is:[/COLOR][/B] ' + dns2,'','',icon,'',fanart)
       
def System():
    addDir('[B][COLOR gold]Thời gian Hệ thống:[/COLOR][/B] ' + systime,'','',icon,'',fanart)
    addDir('[B][COLOR gold]Ngày hệ thống:[/COLOR][/B] ' + sysdate,'','',icon,'',fanart)
    addDir('[B][COLOR gold]Lưu trữ đã sử dụng:[/COLOR][/B] ' + hddused,'','',icon,'',fanart)
    addDir('[B][COLOR gold]Lưu trữ còn trống:[/COLOR][/B] ' + hddfree,'','',icon,'',fanart)
    addDir('[B][COLOR gold]Bộ nhớ trống:[/COLOR][/B] ' + freemem,'','',icon,'',fanart)
    addDir('[B][COLOR gold]Chế độ hiển thị:[/COLOR][/B] ' + screen,'','',icon,'',fanart)
    addDir('[B][COLOR gold]Ngôn ngữ hệ thống:[/COLOR][/B] ' + lang,'','',icon,'',fanart)
    addDir('[B][COLOR gold]Nền tảng hệ thống:[/COLOR][/B] ' + plat,'','',icon,'',fanart)
